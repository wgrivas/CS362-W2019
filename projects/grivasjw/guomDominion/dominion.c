...
void council_roomCard(int currentPlayer, struct gameState *state, int handPos)
{
  ...
  {
        drawCard(i, state);     //bug introduced: no if statement to check for current player
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
      ...
      adventurerCard(drawntreasure, currentPlayer, state, z, temphand);
      return 0;
